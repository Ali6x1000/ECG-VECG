...
# ==========================================
# Helper: Extract & SMOOTH the Signal
# ==========================================
def extract_and_smooth_signal(img_tensor, window_length=15, polyorder=3):
    # 1. Convert to Numpy
    img = (img_tensor.squeeze().permute(1, 2, 0).cpu().numpy() + 1) / 2
    img = np.clip(img, 0, 1)
    
    # 2. Invert (Black lines = Bright signal)
    grayscale = img.mean(axis=2)
    inverted = 1.0 - grayscale
    
    # 3. Raw Extraction (The Noisy Version)
    height = inverted.shape[0]
    raw_signal = height - np.argmax(inverted, axis=0)
    
    # 4. THE FIX: Smoothing
    # window_length must be odd. 15 is approx 0.15s (good for ECG)
    try:
...
# ==========================================
# Run Plotting with Smoothed Data
# ==========================================
plt.figure(figsize=(12, 6))

# Extract using the NEW function
sig_young = extract_and_smooth_signal(images[0]) # Original
sig_old = extract_and_smooth_signal(images[-1])  # Oldest

# Plot Comparison
plt.subplot(2, 1, 1)
plt.title("Smoothed Voltage Comparison (True Biological Shape)", fontsize=14, fontweight='bold')
plt.plot(sig_young, label='Young (Original)', color='blue', linewidth=2, alpha=0.8)
plt.plot(sig_old, label='Old (Generated)', color='red', linewidth=2, alpha=0.8)
plt.ylabel("Voltage (Smoothed)")
plt.legend()
plt.grid(True, alpha=0.3)

# Plot Difference
diff = sig_old - sig_young
plt.subplot(2, 1, 2)
plt.plot(diff, color='purple', linewidth=1.5)
plt.fill_between(range(len(diff)), diff, 0, where=(diff>0), color='red', alpha=0.3, label='Taller/Wider')
plt.fill_between(range(len(diff)), diff, 0, where=(diff<0), color='blue', alpha=0.3, label='Lower/Narrower')
