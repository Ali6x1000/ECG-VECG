...
PICKLE_CACHE_PATH = ""
LOCAL_PICKLE_CACHE_PATH = "/content/ecg_vcg_image_cache_hr_1x224x224.pkl"

# --- New Mode-Specific Cache Paths ---
# These will be generated based on the TRAINING_MODE
IMAGE_CACHE_PATH = os.path.join(DRIVE_BASE_PATH, f"ptbxl_{TRAINING_MODE}_cache.npy")
STATS_PATH = os.path.join(DRIVE_BASE_PATH, f"ptbxl_{TRAINING_MODE}_stats.npz")
LOCAL_IMAGE_CACHE_PATH = f"/content/ptbxl_{TRAINING_MODE}_cache.npy"
...
if os.path.exists(DRIVE_BASE_PATH):
    ...
    # Copy the source pickle file
    # if os.path.exists(PICKLE_CACHE_PATH):
    #     copyfile(PICKLE_CACHE_PATH, LOCAL_PICKLE_CACHE_PATH)
    #     print("✅ Pickle cache copied.")
    # else:
    #     print(f"⚠️ {PICKLE_CACHE_PATH} not found on Drive! Cannot proceed.")
    #     # Stop execution if source file is missing
    #     raise FileNotFoundError(f"Source pickle file not found: {PICKLE_CACHE_PATH}")

    # Check if the mode-specific cache already exists on Drive
    if os.path.exists(IMAGE_CACHE_PATH):
        copyfile(IMAGE_CACHE_PATH, LOCAL_IMAGE_CACHE_PATH)
        print(f"✅ Found and copied existing {TRAINING_MODE} cache.")
    else:
        print(f"⚠️ {TRAINING_MODE} cache not found on Drive. It will be generated locally.")
else:
    print("⚠️ Google Drive not mounted or path is incorrect. Using local paths.")
    LOCAL_PICKLE_CACHE_PATH = PICKLE_CACHE_PATH # Assume local path
...
try:
    ...
    print(f"❌ ERROR: Metadata file not found at {META_CSV_PATH}")
...
if not os.path.exists(LOCAL_IMAGE_CACHE_PATH):
    print(f"🛠️ {TRAINING_MODE} image cache not found. Generating from pickle file...")
    ...
    try:
        ...
        print(f"❌ ERROR: Local pickle file not found at {LOCAL_PICKLE_CACHE_PATH}")
        raise
    ...
    print(f"✅ {TRAINING_MODE} cache generated.")
    if missing_count > 0:
        print(f"⚠️ Warning: {missing_count} records from metadata had no matching entry in the pickle file.")
else:
    print(f"✅ {TRAINING_MODE} image cache found. Skipping image generation.")
...
if not os.path.exists(STATS_PATH):
    print(f"🛠️ Global stats for {TRAINING_MODE} not found. Calculating...")
...
print("\n🛠️ Building model with augmentation layers...")
...
x = keras_cv.layers.RandomGaussianBlur(kernel_size=3, factor=(0.1, 0.5))(x)
...
# --- (THE REST OF YOUR SCRIPT IS IDENTICAL) ---
